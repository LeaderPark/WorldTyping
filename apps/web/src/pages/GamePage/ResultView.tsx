...
export function ResultView({
  ...
  return (
    <div className="wt-result-view" data-testid="result-view">
      {/* WT-DC-04(⑥): 완주=입국 완료 / 게임오버=라이프 소진 분기(디자인 L356). 입장 페이드는
          globals .wt-result-view__title(wt-intro-fade 350ms, reduced-motion 정지). */}
      <h1 className="wt-result-view__title" tabIndex={-1}>
        {result.outcome === 'completed' ? t('result.title') : t('result.outcome.gameover')}
      </h1>
