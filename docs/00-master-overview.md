...
| D48 | LCP 판정 환경·정적 셸 독립 페인트 보장 | D47 이행 후에도 로컬 Lighthouse(simulate)가 2490/2640ms 바이모달 — React 커밋이 같은 프레임에 정적 셸을 교체하면 독립 페인트 미기록(레이스, 실측 입증) | **2단 확정**: ① main.tsx의 createRoot 초기 render를 rAF 1프레임 지연해 정적 셸 독립 페인트 보장(D47의 "자연 대체" 문구 개정 — 명시 승인). ② 규범 게이트는 스펙 그대로 **Lighthouse CI(표준 러너, <2.5s assert)** — 로컬 dev 머신 측정은 스모크(5회 전부 ≤2.55s 밴드 + 수치 보고 의무)로 재정의, D47의 로컬 중앙값<2.4s 요구 폐기. 원격 CI 활성화 시 실판정 (M5 확정, 2026-07-22) |
