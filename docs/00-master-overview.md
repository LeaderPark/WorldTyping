...
| D110 | 여권 업적 도감 — 24종 상시 표시, 미달성 흑백/달성 컬러 | 사용자: 전체 업적을 미리 보여주고 달성 시 활성화되는 도감 UX ↔ 기존 여권은 달성분 중심 | **확정(서버 무변경)**: ① 데이터 = 기존 GET /users/:id/passport의 unlocks(type 'achievement') 그대로 소비 — 스키마·프로토콜·마이그레이션 0 ② 표시 카탈로그 = 클라 상수(features/passport/achievements-catalog.ts, 서버 lib/achievements와 id·순서 파싱 대조 테스트로 잠금) + i18n `achv.{kebab-id}.name/desc` 48키(키 문법이 kebab-case 강제라 밑줄→하이픈 변환 함수로 고정) ③ UI = 여권 스프레드 전폭 3~5열 그리드, 미달성 = **아이콘만** grayscale+0.45(텍스트는 --text-muted로 대비 보존 — .wt-token--locked 원칙), 달성 = 원색+금테, 조건 문구 상시 노출, "업적 {n}/{total}" 카운트, sr-only+data-unlocked a11y ④ 숨김 업적 없음 확정(문서·서버 카탈로그에 hidden 개념 부재 — 전부 공개형) (WT-PASSPORT-DEV-1, 2026-07-25) |
| D112 | IME 재정렬 변형 봉인 — one-shot 폐기, staleEcho 국가 내 상시 무장 (D98·D104·D106 최종 개정, **Fable 직접 조사·구현 — 리드 지시**) | D106 배포 후에도 '도대' 잔존(빈도만 감소). git log 전수 재조사로 확정한 원인: D70~D106의 모든 방어가 "에코가 사용자 타보다 먼저 온다"를 가정 — 실기기에는 **사용자 첫 자모(ㄷ, keydown 상관)가 먼저 도착해 one-shot을 소진**시킨 뒤 IME가 다음 스냅샷에서 옛 음절을 병합('도대')하는 순서 역전이 존재(사용자 증언 "맨 처음(D84 직후)엔 정상"과 정합 — 이후 개정들이 one-shot 의존을 심화) | **확정**: ① one-shot 소거 전면 폐기 — staleEcho는 국가 전환 내내 무장 유지(해제 = 다음 flush의 재무장/클리어) ② 안전 근거: 모든 삼킴·스트립이 [구조 게이트(꼬리/접두 일치) ∧ 의미 중재(전체 판정 MISS일 때만) ∧ 예산 3회]를 요구 → 정상 진행(PREFIX/EXACT)은 어느 분기에도 불가침 ③ 상시 무장으로 새로 노출되는 분기 (2)(Gboard 옛 전체값 접두)에 (3)과 동일한 의미 중재 추가 — **인도→인도네시아**처럼 새 국가명이 옛 국가명으로 시작하는 genuine 진행 보호(P9 잠금) ④ 부수 효과: 이미 누수된 버퍼도 다음 스냅샷에서 자가 치유(P10) ⑤ P2 계약 반전(구 "소진 후 무방비"가 곧 결함이었음), 재현 P8(수정 전 실패). engine 163/163·커버리지 99.19/92.59·웹 회귀 836 (WT-FIX-IME-D112, 2026-07-25) |
| D111 | chase Phase 1 — 시작 전 '게임 방법' 안내·첫 런 코치마크·phase 오버레이 은닉·레이더 스윕 복원·목표 라벨 | 사용자: "시작 전 충분한 설명" + 플레이 비직관 잔여 ↔ D96 비블로킹·§8.3 HUD 규격·§7.1 브리핑 비컨 | **확정(표시 계층)**: ① 브리핑에 '게임 방법' 5항목(도주/금/수배/체포/등급 — 수치는 KV 튜닝 대상이라 일반 표현, 유일 수치는 구조 상수 '후보 3개국') — 첫 진입 펼침(wt:chase:howtoSeen), 이후 디스클로저 토글. 카드가 role=button이라 **형제 배치**(중첩 인터랙티브 회피, START 동기 focus 계약 보존) ② 첫 런 코치마크 3개(콜아웃→수배 별→홈/레이더, 2.6s 간격, pointer-events:none 완전 비블로킹, wt:chase:tipsSeen) ③ idle·finished phase에서 chase 오버레이 은닉(240ms 페이드, idleSpin과 역동조) — §7.1 "브리핑 중 홈 비컨 선점등"은 이 결정이 우선해 폐기, 체포 마커 팝(§7.6 520ms)은 은닉되지만 풀스크린 체포 연출 2.8s는 별도 레이어라 불변 ④ 수배 점등 레이더 스윕 복원(SVG 42° 부채꼴 600ms, CH-07 축소분 — 오디오 동기, reduced/juice 이중 게이트) ⑤ HUD 목표 라벨(금 찾기↔배송, HUD 바 밖 배치로 h40 규격 불변, textContent 직접 갱신 §4.5) + 배송 상태 홈 비컨/화살표 강조(기존 setCarriedCount 경로). i18n 17키(+)·3키(−) — howto 키는 CI 3단계 제약으로 runTitle 합성형. **entry +0.86KB(i18n 카탈로그 — 규약상 회피 불가, 103.71/170KB)**. 유의: chase.howto.arrestBody 문구가 D94 기본값(50%)과 결합 — 튜닝 시 동반 갱신 (WT-CH-DEV-2, 2026-07-25) |
| D113 | IME 기계 판별 오탐 봉인 — 분기 (0) 중재 MISS-전용 강화 (D112 후속, **Fable 직접 — 리드 지시 유지**) | D112 배포 후 신규 증상 "간헐적으로 처음 친 자음이 사라짐" — keydown 상관(80ms)이 무거운 프레임/기기 지연에서 빗나가면 사용자의 진짜 첫 자음('ㄷ')이 기계 스냅샷으로 오인되는데, 분기 (0)의 구 게이트는 "EXACT만 보호"라 유효 PREFIX인 'ㄷ'을 삼켰다(D112 상시 무장으로 노출 확대) | **확정 — 불가침 원칙 명문화**: 유효 진행(PREFIX/EXACT)은 어떤 판별 실패에도 삼키지 않는다 — 분기 (0)의 의미 중재를 (1)(3)과 동일한 **전체 판정 MISS-전용**으로 강화. 진짜 에코('도'=MISS)는 종전대로 삼킴. 트레이드(명시 수용): 점진 에코의 1조각('ㄷ')이 새 타깃의 유효 접두면 순간 표시될 수 있으나 다음 조각(MISS)에서 즉시 삼켜져 자가 정리 — 잔류('도대')는 계속 불가능. P1 계약 개정(1조각 통과+2조각 삼킴), P5~P7은 MISS-형 픽스처(인도→가나)로 전환. engine 163/163·web 857 (WT-FIX-IME-D113, 2026-07-25) |
| D114 | chase Phase 2-1 — 경찰 활공 연출(표시) + 이동 주기 +20% 감속(심 상수, constantsVersion v2) | 사용자: 경찰 순간이동이 어색·플레이어보다 살짝 느리게 ↔ D91 심 결정성·서버 검증 패리티·상시 rAF 금지 | **확정**: ① 표시 — 경찰 마커 at 변경 시 globe-hop 수학 재사용 대권 활공(duration=min(거리 가중, 최단 틱×0.8 — 상수 연동 클램프), 중도 재타깃은 현 보간점에서 재시작, 활공 중에만 도는 bounded rAF 후 정적 재투영 복귀, reduced/juice 강등 스냅, 미세 기울임 8°만 신규) — 심 무접촉 ② 심 — police 틱 4항목 정확히 ×1.2(chaserBase 4200→5040·perStar 300→360·min 3000→3600·interceptor 4200→5040·heli 1800→2160, 파생 관계 보존, 거리 규칙 3항 불변), simulate 무수정, `CHASE_CONSTANTS_VERSION` 1→2(D93 폴백 확인: 구버전 런은 reject 아닌 practice 강등) ③ 골든 3종 재생성(틱 지연→체포 시각·홉 수 증가 형태 — 정당 귀결), docs/09 §3.4 동기 ④ 운영 각주: v2 배포 시 진행 중 v1 런 정밀 재계산을 원하면 KV `config:chase:v1` 스냅샷 기록(chase-config.ts 헤더 절차) — 현재 chase는 '준비중'이라 실사용 런 없음, 미기록 시에도 practice 강등으로 안전 (WT-CH-DEV-3, 2026-07-25) |
| D115 | chase UI/UX 재설계 — "판단은 카드에서, 지도는 확인": 작전 카드·수배 레이더·마커 위계 | 사용자: 금·경찰 위치와 선택지 인지가 비직관(정보가 회전 지구본 위에만 존재) ↔ 09a 인월드 콜아웃 계약·콜아웃 마운트 후 React 커밋 0·상시 rAF 금지 | **확정(표시 계층, 신규 엔진 이벤트 0)**: ① **작전 카드** — 후보 칩에 인텔 행(💰금 유무/1홉 인접·가치 도트, 🚨위험 강조(D97 danger), 🏠홈 거리 델타 ▼녹/▲회+배송 중 금색 필, 이동 km — 전부 기존 이벤트+전쌍 km 행렬 파생, 명령형 갱신 유지, CHIP_H 64→80) ② **수배 레이더** — 좌하 120px 원형(북고정·log 스케일 2만km 포화): 금(가치별 크기)·경찰(종류 실루엣, ≤3,000km 펄스 — §3.3 도주 임계와 동일)·홈(배송 중 강조), 저빈도 이벤트 갱신만, ★ 상승마다 600ms 스윕(지구본 스윕 D111은 런당 1회 발령 연출 — 포함 관계로 병존, 이관 안 함) ③ 마커 위계 — 금 광선 기둥·경찰 위협 글로우(threat 3단)·플레이 중 195 노드 딤 0.26 ④ **카메라 자동 프레이밍은 드롭(리드 확정)** — 후보는 nearest-12라 플레이어 카메라 전면에 구조적으로 존재(09a §5.2), 원거리 금·경찰 인지는 ②가 해결, 구현 시 4중 구조 충돌(카메라=플레이어 결합·미러 스냅·코어 기체 렌더·D114 활공) 대비 효익 0 ⑤ i18n 5키(aria/툴팁 전용 — 칩 표기는 언어 중립), 신규 토큰 --chase-closer-text(4.9:1). web 888·entry +0.20KB(105.15/170) (WT-CH-DEV-4, 2026-07-25) |
| D116 | IME 기저 붕괴 의미 중재 — base-collapse MISS-전용 강화 (D113 후속, **Fable 직접 조사·구현 — 리드 지시 유지**) | D113 배포 후 신규 증상 "북한→중국에서 '중' 입력 중 ㅈ·ㅜ가 지워지고 ㅇ만 잔존"(간헐) — IME는 분기 (2)(3)이 가상 스트립한 반삽입 접두('한')를 다음 스냅샷에서 스스로 걷어내며 value를 자기 조합 상태 기준으로 재작성할 수 있는데('한ㅈ' 스트립 후 v='주'), 구 base-collapse 분기는 붕괴 잔여의 판정 상태를 보지 않는 **유일한 무중재 삼킴 경로**라 유효 진행(ㅈㅜ)을 settleSwallow(조합 중 blur)로 파괴 → 데싱크된 IME가 다음 자모만으로 새 조합 시작('ㅇ'만 잔존). 3방향 검증(독립 재추적·적대적 반박·대안 스윕 — 셋 모두 동일 분기로 수렴, '주'를 삼킬 수 있는 다른 분기 없음) | **확정**: ① base-collapse에도 (0)(1)(3)과 동일한 **MISS-전용 의미 중재** — 붕괴 잔여가 유효 진행(PREFIX/EXACT)이면 basePrefix만 해제하고 그대로 평가(보존, 삼킴 아님 — staleEcho·예산 불변으로 D112 자가 치유 유지), MISS면 종전 기구(조용한 리셋·재무장) 불변. D113 불가침 원칙의 마지막 누락 분기 봉인 ② 트레이드(D113 동형, 명시 수용): 옛 꼬리가 새 타깃의 유효 접두인 전환에서 순수 에코 붕괴 잔여가 보존될 수 있으나(D98-④ 기수용 누수 클래스의 collapse 확대) 사용자 진행과 값 층 구별 불가라 보존이 정당, 에코 보존 시 계상 미세 인플레이션 가능(addedError 무왜곡 — non-MISS 게이트가 접두 유효성 보장) ③ 트레이스 신설 분기 'base-collapse-genuine'(보존 경로 원격 진단) ④ 계약 반전: ⑤(MISS-형 픽스처로 개정)+⑤b 신설·W-C1 반전·N12 이중 분기·재현 Q1(북한→중국 풀 시퀀스, 수정 전 실패 실측)·Q2(붕괴-EXACT). 유의(검증 잔여): IME가 DOM 값 자체를 옛 음절로 덮어쓰는 대안 변형은 컨트롤러가 보존할 값이 없어 이 픽스 밖 — 재발 시 실기기 wt:imeTrace로 strip→base-collapse(가설) vs swallow-echo 단독(대안) 확증할 것. engine 166/166·커버리지 99.19/92.65·web 888·typecheck·lint (WT-FIX-IME-D116, 2026-07-26) |
