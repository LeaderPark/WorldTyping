...
| 세계일주 (50개국, §11-D2) | 5분~9분 | 주 1~2회 도전하는 마라톤 |
...
> **[docs/00 §11-D2로 대체됨]** 아래 80개국 서술은 초안이다. 확정: **50개국**(`ROUTE_WORLD_TOUR`), 루트 전문·체크포인트(10/20/30/40 + 종착)·경유 도시는 **docs/02 §6이 원천**. 이 절의 수치·루트·체크포인트 도시는 역사적 기록으로만 남긴다.
...
- 목표: 랜딩 → 첫 타이핑까지 **최단 경로 3클릭·15초 이내** (대륙 정규 경로는 4클릭 — docs/00 §11-D36).
...
- 별칭 허용 원칙: ① 공식 명칭("미합중국", "조선민주주의인민공화국"), ② 직전 공식 표기("터키", "체코슬로바키아 ❌ — 소멸국은 불허, 개명 전 표기만 허용: "마케도니아"→북마케도니아 OK), ③ 영문 약칭("USA","UK","UAE","DPRK"), ④ 외래어 표기 흔들림("에티오피아/이디오피아", "말레이시아/말레이지아" — 국립국어원 비표준이라도 통용 빈도 상위는 허용), ⑤ **표시 정식명의 진접두인 별칭 금지**(docs/00 §11-D82) — 표시명 타이핑 도중 조기 EXACT를 유발한다(예: "사우디"⊂사우디아라비아, "DRC"⊂DR Congo — 제거됨).
...
- 스타일: 플랫 벡터 지도(Natural Earth 기반 단순화 폴리곤) + 여권/보딩패스 페이퍼 텍스처. 다크 모드 기본(타이핑 집중), 라이트 모드 옵션.
  - *(개정 — 00 §11-D57, 2026-07-23)*: 기본 테마는 **라이트**(크림 `#f4f5ef`, `:root`), 다크는 `[data-theme='dark']` 옵션으로 강등(설정 토글 동작 보존). 위 "다크 모드 기본" 서술은 D57로 대체된 것으로 읽는다.
