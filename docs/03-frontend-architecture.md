...
> **개정 2(docs/00 §11-D84, 버그 W)** — 위 개정 3의 관문을 확장한다: (a) 재삽입 윈도우 48ms→**150ms**
> (late 재삽입 흡수 — D70 테스트 ③ 기대 개정: ≤150ms 옛-꼬리 전량 재삽입=삼킴, 초과=genuine),
> (b) **부분 꼬리 스트립 신설** — 옛 값의 proper 접미(끝음절, ≥2자모)가 새 입력의 raw 접두로 병합된
> 경우(예: 인도→대한민국 '도대'), **전체 판정이 MISS이고 잔여 판정이 non-MISS일 때만**(의미 중재)
> 그 접미를 basePrefix 기구로 가상 스트립해 잔여만 평가한다(윈도우·재삽입 예산 공유). genuine 해석이
> 유효하면 절대 스트립하지 않는다. 잔여 수용 항목·근거는 docs/00 §11-D84.

> **개정 3(docs/00 §11-D98, 버그 W 재발)** — 개정 2의 **시간 윈도우 게이트를 제거**한다. 실기기 IME
> (Windows MS IME 포함)는 재삽입을 flush 직후가 아니라 **사용자 다음 키스트로크 시점**에 병합
> 발현시키므로 flushAt 앵커 150ms는 구조적으로 뚫린다(국가 전환 후 한 박자 쉬면 '도대' 재발 — 라이브
> 재현). 부분 꼬리 스트립(개정 2-(b))은 의미 중재·예산·≥2자모만으로 동작하고, 전량 에코 삼킴은
> 윈도우 내 무중재 fast-path 유지 + **윈도우 밖에서는 전체 판정 MISS일 때만** 삼킨다(genuine
> PREFIX/EXACT 우선). 노출 유계성은 staleEcho one-shot(국가당 첫 비어있지 않은 스냅샷 1회)이, fail-open은
> 재삽입 예산(국가당 3회)이 담당한다. 잔여 한계: 늦은 lone 에코가 새 타깃의 유효 접두인 경우
> (인도→도미니카 '도')는 미방어 — genuine 우선 원칙.
...
- **렌더 계층**: **canvas**(baseground, `aria-hidden`) — 바다 원판(`--map-ocean`)+림(`--map-rim`)+경위선(`geoGraticule10`,`--map-graticule`)+중립+기본 폴리곤+skipped+solved(대륙색 300ms 알파 램프)+target(대륙색+밝기)+노선 great-circle 아크(케이싱+대륙색, 완주 노선만 — 진행 홉은 앰버 트레일이 대체, §11-D73)+활성 홉 앰버 트레일(점선+글로우 3-패스)+스테이션 도트. 매 프레임 `projection.rotate`로 전 폴리곤 재투영(orthographic이 뒷면 자동 클립). **SVG 오버레이**(`.wt-map .wt-globe__overlay`) — 숨김 ledger(`[data-layer=solved|skipped] [data-country]` = e2e 셀렉터 보존, d 없는 path) · 타깃 펄스 링 · 체크포인트 링 · 웨이포인트 라벨 3 · 비행기 g · 파티클 g. 팔레트는 사전 해석(루프 내 getComputedStyle 금지, §4.5).
- **카메라 = 홉 추적**: `moveVehicle(from,to)`가 단일 rAF 루프에서 `projection.rotate([-interp(easeInOutCubic(raw))])`로 카메라를 홉 보간 위치에 고정(비행기는 화면 중심에서 lift `scale=0.8+sin(π·raw)·0.85`(정지·스냅 0.8, §11-D73) + `bearingDeg-90` 회전). duration=각거리 가중 550~900ms(인게임 불변) — 홈 데모만 `moveVehicle(...,{durationMs})`로 순항 2.6s(`HOME_GLOBE_HOP_DURATION_MS`). **홉 중 재호출=선점**(잔여 아크 즉시 완성, 현 위치→새 목적지 리타깃, 큐잉 없음). `drawRouteSegment`=노선 원장 등록(직후 동일 from/to 홉이 진행 소유). `flyTo`=세트 anchor `geoCentroid`로 회전만(줌 불변) — finished 리빌(1200ms)·스킵 카메라 이징(400ms).
- **비행기 실루엣·트레일(참조 정합, §11-D73)**: 비행기 = 리드 참조 프로토타입(globe-flight.html)의 24×24 제트 path 원문 + 정적 `transform="rotate(90) translate(-12 -12)"`(노즈 +x 정렬 — bearing−90 규약 불변), 색 = 신규 토큰 `--globe-plane-fill`(#fff)·`--globe-plane-stroke`(#274690, 양 테마 공통) + stroke 1.4 + drop-shadow. 활성 홉은 비행 위치를 누적해 앰버 점선+글로우 트레일(`--globe-trail`·`--globe-trail-glow`, canvas 3-패스 — `ctx.filter`/`shadowBlur` 금지로 폭 계층 에뮬레이트)로 그리고, 도착 후 600ms 선형 페이드아웃(`rampUntil` 구동) 하며 그 아래로 완주 대륙색 아크가 확정된다. reduced-motion/juice≥1·`from===to` 스냅·reset은 트레일 무생성·즉시 해제.
...
- **idle spin(리드 확정 ②)**: 보딩(phase idle)·결과(finished) 배경에서만 0.55°/s 자전 ON(Tweak B — 초기 ~1.2°/s가 "확확 도는" 느낌이라 리드 요청으로 절반↓, 25fps 스로틀), countdown·playing 중 OFF(playing 중 정지 = canvas 재그리기 0 = 입력 핫패스 무비용). reduced-motion/juice≥1이면 홉·스핀 생략(즉시 스냅).
