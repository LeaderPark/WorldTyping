...
### 1.6c 심야 배치 (2026-07-25 밤 — `main`=`d6af0e8`)
- **IME 최종 2단**: D112(재정렬 변형 — one-shot 폐기·상시 무장, Fable 직접) + D113(kd 오탐 시 첫 자음 소실 — 전 분기 MISS-전용 중재 통일, 유효 진행 불가침 원칙 명문화).
- **골드 러너 집중 디벨롭**(전부 '준비중' 상태에서, /play/chase 직링크로 테스트 가능): D111(게임 방법 5항목+코치마크+오버레이 은닉+스윕 복원+목표 라벨) · D114(경찰 활공 연출+틱 ×1.2 감속, constantsVersion v2 — 구버전 런은 practice 강등으로 안전) · **D115(작전 카드·수배 레이더·마커 위계 — "판단은 카드에서" 재설계, 카메라 프레이밍은 4중 구조 충돌로 드롭 확정)**.
- **기타**: D110 여권 업적 도감 · 로비 필터/퀵매치 제거 · 마스코트 파비콘 · 브랜드 홈 링크 · 여권 게이트 v3.
- 잔여: chase 밸런스 튜닝 계속(KV) → CH-10 E2E(E-C 3종) → '준비중' 해제(dd04141 revert). push는 계속 fitfunsJinWoo 403 — 사용자 채널로 원격 동기 중.
...
### 5.1 M6 이후 후속 리드 태스크 (D57~D79) — D77까지 main 머지·라이브 배포 완료 (D78 Tweak S·D79 Tweak T는 이 배치로 머지·배포)
...
| 페이지 크롬 통일 (WT-CHR-01·Tweak F·G·H·I, D74) | 브라우징 페이지 폭·헤더·좌상단 브랜드·법적 정렬/버전 통일. **F(폭)** — 신규 토큰 `--wt-page-max: 42rem`(tokens.css) + 공유 클래스 `.wt-page`(globals.css §2.3), 브라우징 페이지 루트에 `wt-page` 부여 + 페이지 전용 폭/마진/패딩 삭제(홈 `.wt-home__content`는 max-width만 토큰화, 시각 델타 0), `.wt-lobby,.wt-room` 규칙 분리(로비만 `.wt-page`로, `.wt-room` 48rem 불변), `.wt-mode-select__cards` minmax 14→12rem(42rem 3열 유지), Tweak J width:100% 핫픽스 제거. **G+I(헤더·브랜드)** — 신규 `components/PageHeader.tsx`+`BrandMark.tsx`(✈+app.title), 하위 페이지 헤더를 PageHeader로 통일(back 부재 페이지에 rank/passport/daily/privacy/lobby-back 신규), 홈은 자체 헤더 유지+좌상단 BrandMark 추가+데일리 뱃지 우측 이동(계약 불변), 로비 TopBar 배선 해제(파일 존치). **H(법적)** — `.wt-legal-modal{text-align:left}` 1규칙(footer 모달 좌정렬), privacy.{ko,en}.md v1.1→v1.0 + 부칙 개정이력 표 삭제(terms 포맷 통일). 신규 i18n 키 0. 표시 계층만(판정·점수·프로토콜·엔진·Tweak C·D72·D73 불변). docs 동기: §11-D74·03 §4.2 |
| 프롬프트 캡슐 고정 폭 (Tweak Q·R, D77) — **main 머지 대기** | 인게임 프롬프트(흰 캡슐)의 국가명 줄내림·가변 폭·국기 위치 흔들림을 표시 계층에서 해소(판정·점수·프로토콜·엔진·prompt-renderer 불변). **Q(고정 폭·단일행)** — `.wt-prompt__glyphs`/`.wt-prompt-area` `flex-wrap:nowrap` + `.wt-slot{flex:none}` + `.wt-prompt{overflow:clip}`(안전망), 캡슐 `flex:3 1 0`(race 4 1 0)로 폭을 콘텐츠에서 분리(520px 상수), 칼럼 `flex:1 1 0; container-type:inline-size` + 글리프 `font-size:min(clamp×fontScale, 100cqw/adv×0.98)` 폰트 fit. 신규 `features/typing/prompt-advance.ts`(ko 1.14/en 0.78/구분자 0.4/gap 0.14 em advance, 골든 5건 `prompt-advance.test.ts`)를 PromptArea가 국가 전환 리렌더에서 `--wt-prompt-adv`로 주입(키스트로크 경로 무변경). **R(국기 위치 상수)** — `.wt-prompt-area{width:100%}`로 영역이 고정 폭 캡슐 내부를 채워 [국기(flex:none)+칼럼(flex:1 1 0)] free space 0 → 국기 x좌표 콘텐츠 무관 상수. 모바일(≤639px)은 캡슐 국기 숨김 1건(파일 말미)만 추가(광범위 사문 블록 복원 미채택 — 별도 태스크 이관). docs 동기: §11-D77. 커밋 1개(+docs 분리 가능) |
| 싱글 콘솔 라이트 재테마 (Tweak T, D79) — **main 머지 대기** | S4 트랙선택 "여행 설정" 콘솔(`.wt-console`, tsx 소비처 TrackSelectPage 유일)의 검은 배경(--surface-console #14201a + #f8fafc 고정 텍스트)을 라이트 카드로 재테마(표시 계층 전용 — 판정·점수·프로토콜·엔진·마크업·testid 불변). `.wt-console`을 `.wt-card` 레시피(`--surface`+`--text`+`--shadow-card`, padding·tabular-nums 존치)로 개정 + 하드코딩 #f8fafc 및 콘솔 스코프 오버라이드(`.wt-console .wt-kicker`/`.wt-token__label`) 삭제 → 전역 시맨틱 토큰 복귀(양 테마 자동). 대륙 6색 원은 원색 fill 유지 + 링 `color-mix(원색 75%, black)`(흰 배경 경계 식별 ≥3.36:1), 중립 T칩은 base(--surface-sunken/--border)+`--text`, 라벨·기록은 `--text-muted`, dot `color-mix(--grade-c 75%, black)`, 행 호버 `color-mix(--text 7%, transparent)`. `--surface-console` 토큰 값·인게임 칩(.wt-strip__transfer/.wt-cp-banner__pill)·멀티 포토피니시(.wt-race-result__photo)는 무접촉(주석만 개정). contrast-check.ts 신규 8항목(중립 칩 4.5:1 + 링 6종·dot 3:1) 추가(기존 무수정). docs 동기: §11-D79. 커밋 1개(+docs 분리 가능) |
| 헤더 정합 (Tweak L·M, D75) | D74 후속 리드 피드백(표시/a11y 계층 전용). **L(2행 nav 폐지)** — PageHeader 2행 `.wt-page-header__nav`(뒤로가기 `.wt-nav-back` `<Link>` + `h1.wt-page-header__title`) 시각 폐지, `.wt-page-header` 기하를 `.wt-home__header`와 픽셀 동일(flex·flex-wrap·space-between·gap 1rem)로 + 우측 액션 `.wt-icon-tile`(테마 토글)을 34px 컴팩트로 맞춰 홈↔하위 헤더 top/height 24/34px 완전 일치(Playwright 측정 검증). 뒤로가기 링크 렌더 제거(`back` prop·`PageHeaderBack` 타입·`{page}-back` testid 10종·`.wt-page-header__bar/__nav/__title`·`.wt-nav-back` CSS·`nav.back.mode` 소비 제거, `nav.back.home`은 TopBar 존치라 카탈로그 유지). title은 **sr-only h1(tabIndex=-1)**로 보존 — router.test 첫 h1 순회·useRouteFocus·axe(E10) 계약 불변, DailyPage/LobbyPage는 title 미전달(자체 콘텐츠 h1). **M(홈 데일리 뱃지 제거)** — `home-daily-badge` `<Link>` 삭제(데일리 진입은 메뉴 카드 home-card-daily 유지), 뱃지 전용 state dailyNo·alreadyPlayed·fetchDailyToday/fetchDailyMe/ensureSession 배선·`placeholderDailyNumber()`·`.wt-home__daily-badge` CSS 정리(홈 티커·펄스·언어/사운드 토글 불변). 신규 i18n 키 0. 표시/a11y 계층만(판정·점수·프로토콜·엔진·법적·비행 연출 불변). docs 동기: §11-D75·03 §4.2 |
| 인증 split-brain 봉인 (WT-Z2-FIX, D86) — **main 머지 대기** | 버그 Z2("구글 로그인 후 멀티에서만 로그인 필요")를 순수 클라 상태 계층에서 봉인(서버·판정·점수·프로토콜·엔진·마이그레이션·게스트 100% 플레이·`bearerToken()` 계정>게스트 폴백 전부 불변). **F4** — `net/api-client.ts` `setAuthToken`을 try/catch+read-back 검증(void→boolean), `stores/auth.ts` `login()`이 토큰 영속 성공을 선행 조건으로 실패 시 `AuthPersistError` throw(프로필 미설정) + LoginModal `auth.storageError` 안내(ko/en 1키 추가). **F1** — `selectIsLoggedIn`을 계정 토큰 실존(`getAuthToken()!==null`)에 종속 + `window 'storage'`/`'focus'` 리스너로 크로스탭 로그아웃/로그인(`persist.rehydrate()`) 전파 + 부팅 rehydrate 고아 토큰 소거. **F2** — 멀티 진입(LobbyPage/RoomPage 마운트) 시 기존 `GET /session/me` 1회 재검증(`verifyAccountSession`, 60s 메모·게스트 no-op, 401만 로그아웃 강등) + `request()`가 계정 토큰 첨부 요청의 401 INVALID_TOKEN을 신규 `onAccountTokenRejected` 시그널로 즉시 로그아웃(게스트 토큰 거부 미발화). 신규 단위 stores/auth.test.ts(split-brain·F4·크로스탭·F2a·F2b) + net/api-client.test.ts(setAuthToken 원자화·onAccountTokenRejected) 추가, ResultView.test.tsx 전체 목 스텁을 신 계약(setAuthToken→true·getAuthToken 토큰·onAccountTokenRejected placeholder)으로 정합(단언 무변경). e2e/기존 단위 스펙 무수정. docs 동기: §11-D86. 커밋 1개 |
| 멀티 닉네임·재연결 티켓 (WT-FIX-MP, D88·D89) — **main 머지 대기** | 라이브 멀티 2결함을 순수 클라(apps/web)에서 수정(workers/api·packages/shared(protocol·auth)·moderation·마이그레이션·wrangler 전부 무변경 — `git diff --stat`로 서버 0). **D88(버그 MP-1)** — 멀티 join 신원을 `settings.nickname`→`GUEST_` 폴백(서버 moderation이 `guest_` 예약어로 차단 → 방장 포함 전원 슬롯 미형성)에서 `useAuthStore.nickname`(계정=Google 이름) 단일 출처로 전환(폴백 'PLAYER' 상수). **수동 닉 입력 플로우 전면 제거**: NicknameGate(ResultView)·checkNickname/putNickname(api-client)·settings.nickname/setNickname·run-submit body nickname·`result.nickname.*` i18n(ko/en 4키 대칭 삭제). 탑승권 승객명(BoardingPass)·RankPage 고정 내 순위 행은 계정 닉 우선 + 게스트 GUEST_ **표시 전용** 폴백 유지(서버 전송 없음). 서버 `users.nickname`·routes/nickname.ts 존치. **D89(버그 MP-2/2b)** — WS 티켓 1회용(60s TTL)인데 재연결이 동일 URL(죽은 티켓) 재사용 → DOA 루프. `WsManager.connect(url, reissueUrl?)`+`ReconnectAbortError`+epoch 가드 확장, useMultiplayer가 재연결마다 `POST /rooms/:code/join` 재호출로 신규 티켓 발급(VITE_WS_BASE E2E는 정적 URL 프로바이더로 E7 계약 보존). 터미널 5코드(ROOM_NOT_FOUND/ROOM_IN_PROGRESS/ROOM_FULL/LOGIN_REQUIRED/INVALID_TOKEN)는 잔여 시도 없이 failed+사유별 기존 i18n 키(신규 키 0). WAITING resume 거부(AUTH_FAILED) 시 같은 소켓 무-resume hello+join 1회 조용한 재시도. 백오프 0.5→8s·최대 5회 불변. 신규 단위: ws-manager.test(재발급 5케이스)·useMultiplayer.test(터미널 분류·hello 재시도 순수 헬퍼)·RoomPage.gate.test(신원=계정닉·실패화면 사유). e6/e7/e1/e-auth 스펙·mock 무수정 그린. 후속 분리: O1(레이스 중 라이브 grace 복구, 서버 최소 변경)·WT-INV-ROOMNULL(config-null 404 근본원인). docs 동기: §11-D88·D89·03 §6.1·05 §7.2. 커밋 1개 |
| 진행바 고정 (Tweak S, D78) — **main 머지 대기** | 인게임 상단 ProgressLine 진행바 길이가 다음국명·카운트 자릿수마다 가변이던 문제를 표시 계층에서 해소(판정·점수·프로토콜·엔진·prompt-renderer 불변). `.wt-progress-line__next`(progress-next testid + `t('hud.next')`) 블록·`nextCountryName` prop 전면 제거(ProgressLineProps·GameAppBarProps·GameView `next`/`nextName` 도출 정리) + `.wt-progress-line__count`에 `min-width:3.25rem`("50 / 50" 수납)·`text-align:center` 부여로 카운트 열을 상수 폭화 → 레일(flex:1 1 auto) 픽셀 길이 고정. `.wt-progress-line__next` CSS 3규칙(base·모바일·race variant) 제거, i18n `hud.next` ko/en 대칭 삭제(`hud.announceNext`는 GameView 스크린리더 낭독 — 존치). progressbar role/aria·progress-line/progress-count/progress-ack-ghost/progress-ghost-marker testid·도트 계약 불변. ProgressLine.test progress-next 단언 제거. 신규 i18n 키 0(1키 삭제). docs 동기: §11-D78. 커밋 1개(+docs 분리 가능) |
