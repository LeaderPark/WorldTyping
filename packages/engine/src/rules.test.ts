...
import { timeLimitMs as sharedTimeLimitMs, type Country } from '@wt/shared';
import {
  continentRules,
  createModeRules,
  dailyRules,
...
describe('tierRules (docs/01 §7.1 티어별: 서바이벌, 라이프 3, 제한시간 §7.2)', () => {
  ...
  it('제한시간은 @wt/shared 수식 위임 + 첫 국가 ×2', () => {
    expect(r.timeLimitMs(USA, 1)).toBe(4100);
    expect(r.timeLimitMs(USA, 0)).toBe(8200); // 첫 국가 ×2
    expect(r.timeLimitMs(USA, 0)).toBe(2 * r.timeLimitMs(USA, 1)!);
    expect(r.timeLimitMs(USA, 2)).toBe(sharedTimeLimitMs(USA, 2, 'ko'));
  });
  it('en 바인딩도 각자 lang으로 계산', () => {
    const en = tierRules('en');
    expect(en.timeLimitMs(USA, 1)).toBe(sharedTimeLimitMs(USA, 1, 'en'));
  });
  it('onSkip → 라이프 −1', () => {
...
describe('createModeRules 팩토리', () => {
  ...
  it('tier/daily는 lang 바인딩을 반영', () => {
    expect(createModeRules('tier', 'en').timeLimitMs(USA, 1)).toBe(sharedTimeLimitMs(USA, 1, 'en'));
  });
});
