...
// satori/wasm에 의존하지 않는 순수 PNG(Node zlib). 1200×630 대각 그라디언트(브랜드 다크).
...
function buildFallbackPng() {
  const W = 1200;
  const H = 630;
  // 브랜드 팔레트(layout.ts OG_COLORS와 동조): bg0=#0b1220 → bg1=#111c33 대각 그라디언트.
  const c0 = [0x0b, 0x12, 0x20];
  const c1 = [0x11, 0x1c, 0x33];
...
function writeFallbackPng() {
  ...
  const ts =
    ...
    `// spec: docs/06 §9.1(렌더 실패·404 셸용 정적 폴백 OG PNG, 500 금지). 1200×630 브랜드 다크.\n` +
    `const BASE64 =\n  '${base64}';\n\n` +
