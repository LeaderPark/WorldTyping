// spec: docs/00 §11-D12(안티치트 임계 단일 config:anticheat KV 통합 — 확정값), docs/04 §6.2(파이프라인
//       임계)·§6.5, docs/06 §3.3(휴리스틱 표)·§3.4(inputDigest)·§3.5(섀도우밴 누적 3회) + WT-M3-03
//
...
export async function loadAnticheatConfig(kv?: KVNamespace): Promise<AnticheatConfig> {
  ...
  try {
    ...
    console.error("[wt-api] config:anticheat failed schema validation, falling back", parsed.error);
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error("[wt-api] config:anticheat is not valid JSON, falling back", err);
  }
