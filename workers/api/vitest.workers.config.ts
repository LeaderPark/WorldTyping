...
export default defineWorkersConfig(async () => {
  ...
  return {
    test: {
      ...
      // DO 스토리지를 남기는 테스트는 vitest.do.config.ts(isolatedStorage=false)에서만 실행한다
      // (Windows EBUSY 회피 + 같은 파일 이중 실행 방지, §0.4-7 거짓 그린 방지).
      exclude: [
        ...
        // WT-M6-02: /multi OG SSR이 MatchRoom DO를 건드린다 → do config에서만 실행.
